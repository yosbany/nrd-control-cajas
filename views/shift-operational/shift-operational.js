...
async function loadOperationalView(shiftId) {
  ...
  if (!shiftId) {
    ...
    if (window.navigationService) {
      window.navigationService.switchView('shifts');
    }
    return;
  }
...
export function initializeShiftOperational(shiftId) {
  ...
  if (!shiftId) {
    ...
    window.showError('No se proporcionó ID de turno').then(() => {
      if (window.navigationService) {
        window.navigationService.switchView('shifts');
      }
    });
  ...
  loadOperationalView(shiftId).catch((error) => {
    ...
    window.showError('Error al cargar la vista operativa').then(() => {
      if (window.navigationService) {
        window.navigationService.switchView('shifts');
      }
    });
